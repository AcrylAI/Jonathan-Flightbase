...
def install_front2(params):
    if params["flag"] == "MASTER":
        print("==============FRONT INSTALLING==============")
        get_front_src2()
        front_installer_command = "./install/flight-base/installer.sh -h {api_ip} -p {api_port} -l {front_ip} -s {front_port} -b {branch} -r {protocolConfig} -c {front_build_config} -u {ui_guide}".format(
