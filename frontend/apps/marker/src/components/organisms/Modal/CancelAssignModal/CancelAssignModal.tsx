...
import { MONO205, RED502 } from '@src/utils/color';
import { FilterDataType } from '@src/utils/types/data';

import useT from '@src/hooks/Locale/useT';
import useModal from '@src/hooks/Modal/useModal';

import useDeleteJobAssign, {
  useDeleteJobAssignRequestModel,
} from './hooks/useDeleteJobAssign';
...
const CancelAssignModal = ({
  ...
  const assignMutation = useDeleteJobAssign();
  const onClickClose = () => {
    modal.close();
  };
  const onClickSubmit = async () => {
    ...
    const data: useDeleteJobAssignRequestModel = {
      projectId,
      cancelId,
      notCancelId,
      filter,
        ...
        <Modal.ContentContainer confirm>
          <div className={cx('cancel-container')}>
            <div className={cx('label')}>
              <Sypo type='H2'>
                {t(`modal.cancelAssign.header`, {
                  count: selectedRowCount.toLocaleString('kr'),
                })}
              </Sypo>
            </div>
            <div className={cx('desc')}>
              <Typo type='H4' weight='medium'>
                {t(`modal.cancelAssign.text`)}
              </Typo>
            </div>
            <div className={cx('btn-container')}>
              ...
              <Button
                ...
                customStyle={{ backgroundColor: RED502, border: RED502 }}
                loading={assignMutation.isLoading}
              >
                {t(`component.btn.cancelTheAssignment`)}
              </Button>
            </div>
          </div>
