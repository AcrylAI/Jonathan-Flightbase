...
def get_registry_repositories(address):
    ...
    res = requests.get(catalog_address, verify=False)
    if res.ok:
        try:
...
def get_registry_repository_tags(address, repository):
    ...
    res = requests.get(tag_list_address, verify=False)
    if res.ok:
        try:
            tags = res.json()['tags']
        except ValueError as e:
...
def get_registry_image_size(address=None, repository=None, tag=None, real_name=None):
    ...
    get_data = requests.get(manifest_bytag_address, headers=headers, verify=False).json()
...
def delete_registry_repository_tag(address=None, repository=None, tag=None, real_name=None):
    ...
    headers = {'Accept': 'application/vnd.docker.distribution.manifest.v2+json'}
    try:
        digest = requests.get(manifest_bytag_address, headers=headers, verify=False).headers['Docker-Content-Digest']
    except:
        traceback.print_exc()
        return False #failed
    ...
    try:
        requests.delete(manifest_bydigest_address, headers=headers, verify=False)
    except:
        traceback.print_exc()
