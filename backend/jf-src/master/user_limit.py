...
import traceback
from settings import USER_DOCKER_IMAGE_LIMIT, USER_DATASET_LIMIT, USER_TRAINING_LIMIT, USER_DEPLOYMENT_LIMIT
...
def user_docker_image_limit_check(user_id):
    try:
        result = {
                "current": 0,
                "limit" : USER_DOCKER_IMAGE_LIMIT
            }    
        user_docker_image_list = db.get_user_docker_image(user_id)
...
def user_dataset_limit_check(user_id, workspace_id):
    try:
        result = {
                "current": 0,
                "limit" : USER_DATASET_LIMIT
            }   
        if workspace_id is None:
            user_dataset_list = db.get_user_dataset(user_id)
        else :
            user_dataset_list = db.get_user_workspace_dataset(user_id, workspace_id)
...
def user_training_limit_check(user_id, workspace_id):
    try:
        ...
        if workspace_id is None:
            user_training_list = db.get_user_training(user_id=user_id, only_owner=True)
        else :
            user_training_list = db.get_user_workspace_training(user_id=user_id, workspace_id=workspace_id, only_owner=True)
...
def user_deployment_limit_check(user_id, workspace_id):
    try:
        ...
        if workspace_id is None:
            user_deployment_list = db.get_user_deployment(user_id=user_id, only_owner=True)
        else :
            user_deployment_list = db.get_user_workspace_deployment(user_id=user_id, workspace_id=workspace_id, only_owner=True)
