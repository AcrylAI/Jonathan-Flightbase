...
def check_if_non_existing_user_is_included(user_id_list):
    if user_id_list == []:
        return []
    db_users = db.get_user_name_and_id_list(user_id_list)
...
def get_usergroup_list(page, size, search_key, search_value):
    try:
        usergroup_info_list = db.get_usergroup_list(page=page, size=size, search_key=search_key, search_value=search_value)
        if usergroup_info_list is None:
            return response(status=0, result={"list":[],"total":0})
        total = len(db.get_usergroup_list())
...
def create_usergroup(usergroup_name, user_id_list, description):
    try:
        ...
        insert_usergroup_result, message = db.insert_usergroup(usergroup_name=usergroup_name, description=description)
        ...
        usergroup_info, message = db.get_usergroup(usergroup_name=usergroup_name)
        insert_user_usergroup_list_result, message =  db.insert_user_usergroup_list(usergroup_id_list=[usergroup_info["id"]] * len(user_id_list), user_id_list=user_id_list)
...
def delete_usergroup(usergroup_id_list):
    try:
        delete_result, message = db.delete_usergroup_list(usergroup_id_list=usergroup_id_list)
...
def update_usergroup(usergroup_id, usergroup_name, user_id_list, description):
    try:
        ...
        if usergroup_info["name"] != usergroup_name:
            duple_check, message = db.get_usergroup(usergroup_name=usergroup_name)
            if duple_check is not None:
                return response(status=0, message="Usergroup Name [{}] already exists".format(usergroup_name)) # OK
            
        update_result, message = db.update_usergroup(usergroup_id=usergroup_id, usergroup_name=usergroup_name, description=description)
        ...
        insert_user_result, insert_message = db.insert_user_usergroup_list(usergroup_id_list=[usergroup_id] * len(add_id_list), user_id_list=add_id_list)
        delete_user_result, delete_message = db.delete_user_usergroup_list(usergroup_id_list=[usergroup_id] * len(del_id_list), user_id_list=del_id_list)
